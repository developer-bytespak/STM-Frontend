...
interface BookingStatusIconProps {
  status: 'upcoming' | 'active' | 'confirmed' | 'paid' | 'pending' | 'cancelled' | 'completed' | 'failed' | 'refunded';
  className?: string;
}

const BookingStatusIcon: React.FC<BookingStatusIconProps> = ({ status, className = "w-4 h-4" }) => {
  ...
  return <>{iconMap[status] || <div className={className} />}</>;
};
