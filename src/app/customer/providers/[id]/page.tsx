...
export default function ProviderDetailPage() {
  ...
  useEffect(() => {
    // Find provider by ID
    const foundProvider = dummyProviders.find(p => p.id === params.id);
  ...
  if (!provider) {
    return (
      <div className="text-center py-12">
        ...
        <button
          onClick={() => router.push('/customer/search-providers')}
                ...
                {Object.entries(provider.workingHours).map(([day, schedule]) => (
                  <div key={day} className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-gray-600">
                      {schedule.isWorking ? `${schedule.startTime} - ${schedule.endTime}` : 'Closed'}
                    </span>
                  </div>
                ))}
              </div>
